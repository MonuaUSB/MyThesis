...
def load_data():
    ...
    db.part.delete_many({})
    db.dates.delete_many({})
    db.supplier.delete_many({})
    db.customer.delete_many({})
    db.lineorder.delete_many({})

    print("Загрузка данных в коллекцию: part")
    ...
    print("Загрузка данных в коллекцию: dates")
    with open(os.path.join(SSB_DATA_DIR, "date.tbl"), "r") as f:
        ...
        for line in data:
            ...
            if len(fields) != 17:  # Проверка на корректное количество столбцов
                ...
                continue
            dates_data.append({
                "d_datekey": int(fields[0]),
                "d_date": fields[1],
                "d_dayofweek": fields[2],
                "d_month": fields[3],
                "d_year": int(fields[4]),
                "d_yearmonthnum": int(fields[5]),
                "d_yearmonth": fields[6],
                "d_daynuminweek": int(fields[7]),
                "d_daynuminmonth": int(fields[8]),
                "d_daynuminyear": int(fields[9]),
                "d_monthnuminyear": int(fields[10]),
                "d_weeknuminyear": int(fields[11]),
                "d_sellingseason": fields[12],
                "d_lastdayinweekfl": int(fields[13]),
                "d_lastdayinmonthfl": int(fields[14]),
                "d_holidayfl": int(fields[15]),
                "d_weekdayfl": int(fields[16])
            })
        if dates_data:
            db.dates.insert_many(dates_data)

    print("Загрузка данных в коллекцию: supplier")
    ...
    print("Загрузка данных в коллекцию: customer")
    with open(os.path.join(SSB_DATA_DIR, "customer.tbl"), "r") as f:
        ...
        for line in data:
            ...
            customer_data.append({
                ...
                "c_region": fields[5],
                "c_phone": fields[6],
                "c_mktsegment": fields[7]
            })
        if customer_data:
            db.customer.insert_many(customer_data)

    print("Загрузка данных в коллекцию: lineorder")
    with open(os.path.join(SSB_DATA_DIR, "lineorder.tbl"), "r") as f:
        ...
        for line in data:
            ...
            if len(fields) != 17:  # Проверка на корректное количество столбцов
                ...
                continue
            lineorder_data.append({
                "lo_orderkey": int(fields[0]),
                "lo_linenumber": int(fields[1]),
                "lo_custkey": int(fields[2]),
                "lo_partkey": int(fields[3]),
                "lo_suppkey": int(fields[4]),
                "lo_orderdate": int(fields[5]),
                "lo_orderpriority": fields[6],
                "lo_shippriority": int(fields[7]),
                "lo_quantity": int(fields[8]),
                "lo_extendedprice": int(fields[9]),
                "lo_ordtotalprice": int(fields[10]),
                "lo_discount": int(fields[11]),
                "lo_revenue": int(fields[12]),
                "lo_supplycost": int(fields[13]),
                "lo_tax": int(fields[14]),
                "lo_commitdate": int(fields[15]),
                "lo_shipmode": fields[16]
            })
        if lineorder_data:
